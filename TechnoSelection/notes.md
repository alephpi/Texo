...
词表转换能使验证损失达到0.28，复杂公式（CPE）测试集上BLEU达到0.82，编辑距离达到0.13

但是我们需要注意的是，BLEU和编辑距离的“降低”其实并不能直接评判两种蒸馏方法的好坏，因为词表转换完全去除了空格，而子集蒸馏的词表对空格的正确预测一方面提高了BLEU分中n-gram的匹配率，另一方面子集蒸馏的标签长度大约为词表转换的标签长度两倍，因此也提高了编辑距离的分数（后者会对长度作标准化）。然而我们知道，对空格的正确预测实际上对latex文本的预测没有任何实质性变化，反而会使推理速度变慢。更为公平合理的评价应该使用CDM，但我们精力有限，就不测了。